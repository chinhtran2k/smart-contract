...
abstract contract ERC735 {
    ...
    event ClaimAdded(bytes32 indexed claimId, string indexed claimKey, address indexed issuer, uint256 signatureType, bytes32 signature, bytes claim, string uri);
    event ClaimAdded(bytes32 indexed claimId, string indexed claimKey, uint256 scheme, address indexed issuer, bytes signature, bytes data, string uri);
    ...
    function getClaim(bytes32 _claimId) public virtual returns(string memory claimKey, uint256 scheme, address issuer, bytes memory signature, bytes memory data, string memory uri);
