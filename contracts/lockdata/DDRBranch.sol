...
contract DDRBranch is ERC721Base, IDDRBranch, IMerkleTreeBase {
    ...
    function lockDIDByMerkleTree(address patientDID) private onlyOwner returns (bytes32 rootPatientNodeId, bytes32 rootPatientHash){
        uint256[] memory listTokenDDROfPatient = _DDR.getListDDRTokenIdOfPatient(patientDID);
        ...
        if ((listDDRLength % 2) == 1) {
            ...
            templistDDROfPatient = copyArrayToArrayUINT256(listTokenDDROfPatient, templistDDROfPatient);
    ...
    function setLockInfo(uint256 tokenId, address patientDID, bytes32 rootPatientHash) internal {
        bytes32 newHashValue = keccak256(abi.encodePacked(patientDID, rootPatientHash, tokenId));
        ...
        emit DDRBranchLockTokenMinted(tokenId, patientDID, rootPatientHash, newHashValue);
    }
    ...
    function getHashDDROfPatient(address patientDID) public view returns (bytes32) {
        return _hashDDROfPatients[patientDID];
    }

    function getTokenIdRootHashDDR(uint256 tokenId) public view returns (bytes32) {
