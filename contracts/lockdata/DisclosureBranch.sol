...
contract DisclosureBranch is ERC721Base, IDisclosureBranch, IMerkleTreeBase {
    ...
    function lockDIDByMerkleTree(address patientDID, address providerDID) private onlyOwner returns (bytes32 rootProviderNodeId, bytes32 rootProviderHash){
        uint256[] memory listTokenIdDisclosure = _DDR.getListDDRTokenIdOfProvider(providerDID, patientDID);
        ...
        if ((listDDRLength % 2) == 1) {
            ...
            templistDisclosure = copyArrayToArrayUINT256(listTokenIdDisclosure, templistDisclosure);
    ...
    function setLockInfo(uint256 tokenId,address patientDID, address providerDID, bytes32 rootDisclosuretHash) internal {
        bytes32 newHashValue = keccak256(abi.encodePacked(providerDID, rootDisclosuretHash, tokenId));
        ...
        emit disclosureLockTokenMinted(tokenId, providerDID, rootDisclosuretHash, newHashValue);
    }

    function mint(address patientDID, address providerDID, string memory uri) public onlyOwner returns(uint256){
    ...
    function getListHashDisclosureOfProvider(address providerDID) public view returns (bytes32[] memory) {
        return _listHashDisclosureOfProvider[providerDID];
    }

    function getTokenIdRootHashDisclosure(uint256 tokenId) public view returns (bytes32) {
    ...
    function getListTokenId(address patientDID) public view returns (uint256[] memory){
        return _listTokenId[patientDID];
    }

    function getListRootHashDisclosure() public view returns (bytes32[] memory) {
